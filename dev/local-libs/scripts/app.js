...
(function () {
  ...
  function MockLiveHTMLController($rootScope, $window, $scope, $element, $mdCompiler, datasourceSolverService, httpService, interactionService, utilsService, urlParamService, filterService, $translate, $q, $http, $log) {
    ...
    function addSourceFile(contentcode){
      return contentcode + "\n//# sourceURL=" + $window.location.href + "templates/" + vm.id + ".js";
    }

    function baseCompile() {
      eval(addSourceFile(vm.livecontentcode));
      $mdCompiler.compile({
...
$templateCache.put('app/partials/view/tabsnav.html','<md-tabs ng-if="vm.dashboard.pages.length > 1" md-dynamic-height md-border-bottom><span ng-repeat="page in vm.dashboard.pages"><md-tab label=one ng-click=vm.setIndex($index)><md-tab-label>{{page.title | translate}}</md-tab-label></md-tab></span></md-tabs>');
