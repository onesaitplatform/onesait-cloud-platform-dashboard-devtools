...
DatadiscoveryFieldSelectorController.$inject = ["$log", "$scope", "$mdDialog", "$element", "datasourceSolverService", "utilsService"];
  ...
  function DatadiscoveryFieldSelectorController($log, $scope, $mdDialog, $element, datasourceSolverService, utilsService) {
    ...
    vm.subtotalChange =function(index){
      vm.columns.subtotalField = vm.columns.subtotalField==$index?-1:$index;
      vm.refreshModel()
    }

    vm.removeColumn = function(index){
      vm.columns.list.splice(index,1);
      vm.columns.subtotalField=-1;
      vm.refreshModel();
    }
...
(function () {
  ...
  function DatadiscoveryDataDrawController($log, $scope, $element, $timeout, datasourceSolverService, utilsService, $q, $window, urlParamService, filterService) {
    ...
    vm.$onInit = function () {
      ...
      vm.getDataAndStyle({"getDataAndStyleChild":function(){return {data:vm.jexcel.getData(),style:vm.jexcel.getStyle(),headers:vm.getHeaders()}}});
    ...
    vm.redrawView = function () {
      ...
      vm.sendQuery().then(
        ...
        function (data) {
          if (vm.columns.subtotalField != -1 && $scope.pindex === actindex) {
    ...
    vm.getSelectFields = function () {
      return vm.columns.list.map(function (select) {
        return { "field": (select.type != "metric" ? select.field : select.formula), "alias": select.field.replace(/\./g, "_").replace(/\[/g, "_").replace(/\]/g, "") }
      });
    }
    ...
    vm.getSubTotalField = function () {
      return vm.columns.list[vm.columns.subtotalField].field;
    }
    ...
    vm.fromJsonToRowSubTotalArray = function (data) {
      ...
      for (var d in data) {
        var index = vm.getColumnIndex(d);
        if (d !== vm.getSubTotalField().replace(/\./g, "_").replace(/\[/g, "_").replace(/\]/g, "")) {
          rowArray[index] = data[d];
        }
        else {
          rowArray[index] = vm.config.prefixSubtotal.replace("{}",data[d]);
        }
      }
      return rowArray;
    }
    ...
    vm.setSubTotalsJExcel = function (subTotalData, data) {
      ...
      for (var inst in data) {
        if (data[inst][vm.getSubTotalField().replace(/\./g, "_").replace(/\[/g, "_").replace(/\]/g, "")] !== refInstance) {
          ...
          vm.jexcel.insertRow(dataToInsert, indexToInsert - 1 + indexSubTotalData);
          for (var i in dataToInsert) {
            vm.jexcel.setStyle(vm.getJExcelColumnName(parseInt(i) + 1) + (indexToInsert + 1 + indexSubTotalData), 'font-weight', 'bold');
          }
          indexSubTotalData++;
          refInstance = data[inst][vm.getSubTotalField().replace(/\./g, "_").replace(/\[/g, "_").replace(/\]/g, "")];
        }
        indexToInsert++
      }
      var dataToInsert = vm.fromJsonToRowSubTotalArray(subTotalData[indexSubTotalData], refInstance);
      ...
      if(data.length){//With data
        if(!Array.isArray(data[0])){//form array of json (from datasource)
          headers = vm.columns.list.map(function (column) {
            return { title: (!column.alias || column.alias == "" ? column.field.replace(/\./g, "_").replace(/\[/g, "_").replace(/\]/g, "") : column.alias), name: column.field.replace(/\./g, "_").replace(/\[/g, "_").replace(/\]/g, ""), type: ((typeof data[0][column.field] == 'number' || typeof data[0][column.field] == 'integer') ? 'numeric' : 'text') }
          })
        }
        ...
        var headers = vm.columns.list.map(function (column) {
          return { title: (!column.alias || column.alias == "" ? column.field.replace(/\./g, "_").replace(/\[/g, "_").replace(/\]/g, "") : column.alias), name: column.field.replace(/\./g, "_").replace(/\[/g, "_").replace(/\]/g, ""), type: 'text' }
        })
      }
    ...
    vm.resizeJExcel = function(){
      if(vm.columns.list && vm.columns.list.length && vm.status === "ready"){
        $timeout(
          function(){
            //document.querySelector(".jexcel_content").style.height = $element[0].offsetHeight - 87 + "px";
            vm.drawRedrawJExcel(vm.jexcel.getData());
          },300
        )
      }
    }
    ...
    vm.sendQuerySubTotals = function () {
      return vm.from(vm.datasource.identification).filter(getDataStatusFilters()).group(vm.getSubTotalField()).select(vm.getSelectMetrics().concat({ "field": vm.getSubTotalField(), "alias": vm.getSubTotalField().replace(/\./g, "_").replace(/\[/g, "_").replace(/\]/g, "") })).sort(vm.getSubTotalField()).exec()
    }

    vm.sendQuery = function () {
      return vm.from(vm.datasource.identification).filter(getDataStatusFilters()).group(vm.getGroupFields()).select(vm.getSelectFields()).sort(vm.getSortFields()).exec();
    }
...
(function () {
  ...
  EditDashboardController.$inject = ["$log", "$window", "__env", "$scope", "$mdSidenav", "$mdDialog", "$mdBottomSheet", "httpService", "interactionService", "urlParamService", "utilsService", "$translate"];
  angular.module('dashboardFramework')
  ...
  /** @ngInject */
  function EditDashboardController($log, $window,__env, $scope, $mdSidenav, $mdDialog, $mdBottomSheet, httpService, interactionService, urlParamService,utilsService, $translate) {
...
(function () {
  ...
  MockLiveHTMLController.$inject = ["$rootScope", "$window", "$scope", "$element", "$mdCompiler", "datasourceSolverService", "httpService", "interactionService", "utilsService", "urlParamService", "filterService", "$translate", "$q", "$http", "$log"];
  angular.module('dashboardFramework')
    .component('livehtml', {
...
(function () {
  ...
  ElementController.$inject = ["$compile", "$log", "$scope", "$mdDialog", "$sce", "$rootScope", "$timeout", "interactionService", "filterService", "$mdSidenav", "utilsService"];
  angular.module('dashboardFramework')
    .component('element', {
  ...
  /** @ngInject */
  function ElementController($compile,$log, $scope, $mdDialog, $sce, $rootScope, $timeout, interactionService,filterService,$mdSidenav,utilsService) {
...
(function () {
  ...
  MockSocketService.$inject = ["$http", "$timeout", "$log", "__env"];
  angular.module('dashboardFramework')
...
(function () {
  ...
  MockHttpService.$inject = ["$http", "$window", "$log", "__env", "$rootScope", "$q"];
  angular.module('dashboardFramework')
...
(function () {
  ...
  DatasourceSolverService.$inject = ["socketService", "httpService", "$mdDialog", "$interval", "$rootScope", "urlParamService", "$q", "utilsService"];
  angular.module('dashboardFramework')
    .service('datasourceSolverService', DatasourceSolverService);

  /** @ngInject */
  function DatasourceSolverService(socketService, httpService, $mdDialog, $interval, $rootScope, urlParamService, $q, utilsService) {
    var vm = this;
...
(function () {
  ...
  MainController.$inject = ["$log", "$rootScope", "$scope", "$mdSidenav", "$mdDialog", "$timeout", "$window", "httpService", "interactionService", "urlParamService", "gadgetManagerService", "filterService", "utilsService", "$translate"];
  angular.module('dashboardFramework')
    .component('dashboard', {
  ...
  /** @ngInject */
  function MainController($log, $rootScope, $scope, $mdSidenav, $mdDialog, $timeout, $window, httpService, interactionService,urlParamService, gadgetManagerService,filterService,utilsService, $translate) {
    var vm = this;
    vm.$onInit = function () {
     $translate.use(utilsService.urlParamLang());
